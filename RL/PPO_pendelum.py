...
ALG_NAME = 'PPO'
NUM_ENVS = 4 # the number of parallel running envs
TOTAL_TIMESETPS = 150000  # total timesteps for training 25000=832s
TEST_EPISODES = 1  # total number of episodes for testing
NUM_STEPS = 1024 # total number of steps N*M 1024
GAMMA = 0.9  # reward discount 0.99 recommanded
LR_A = 0.001  # learning rate for actor
LR_C = 0.001  # learning rate for critic 0.0003 is a safe default
MINIBATCH_SIZE = 16  # update minibatch size, Recommend equating this to the width of the neural network
...
class PPO(object):
    ...
    def __init__(self, envs):
        ...
        with tf.name_scope('critic'):
            ...
            # input = tf.keras.Input(shape = np.array(envs.single_observation_space.shape).prod(), dtype=tf.float32, name='state')
    ...
    if args.train:
        ...
        for updates in range(num_updates):
            for step in range(NUM_STEPS):  # N envs with M steps
                ...
                values[step] = tf.reshape(value, [-1]) #tf.([x])
                actions[step] = action #tf.([x])
                logprobs[step] = logprob #tf.([x])
                next_state, reward, terminated, truncated, info = envs.step(tf.reshape(action, [-1])) 
                #next_state = [], reward = tf.([x]), done = bool, action[0] = []
                rewards[step] = reward
                next_done = terminated | truncated
            
            # bootstrap value if not done
            next_value = tf.reshape(agent.get_value(next_state), [-1]) #tf.([x])
