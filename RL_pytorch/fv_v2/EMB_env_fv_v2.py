...
class EMB_All_info_Env(gym.Env):
    ...
    def __init__(self) -> None:
        ...
        """
        high = np.array([100, 400, 1e10], dtype=np.float64)
        self.observation_space = gym.spaces.Box(low=-high, high=high, shape=(3,), dtype=np.float64)   
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self.state = np.array([0.0, 0.0, 0.0], dtype=np.float64) #add fv
        self.x1 = 0.0
        self.count = 0
    ...
    def step(self, action):
        ...
        x0, k, sv = self._get_obs() #state from last lime
        x = torch.tensor([x0, self.x1], dtype=torch.float64)
